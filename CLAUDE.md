...
Lattice is a CLI/TUI toolkit for music libraries that treats the filesystem as the source of truth. It reads tags through `mutagen`, walks the tree on every invocation (no index/database), and writes plain `.txt` reports. Python 3.14+, packaged with Hatch (`pyproject.toml`). Runtime deps: `mutagen`, `tqdm`. Current version is `4.6.0` (kept in `src/lattice/config.py`).
...
Three standalone helpers live in `scripts/`, **not** part of the `lattice` package and not exposed through the CLI. They live outside the package because each one mutates state in a way the read-only package contract (spec.md §5) forbids. Don't fold any of them into the `lattice` package without asking.
...
- **`genre_tidy.py`**: two-phase artist→genre authority + reconciler, layered on top of `retag.py`. `build` scans through lattice (read-only, importing `library._scan_album_dirs`) and writes an editable `genre_map.tsv`: one `Artist<TAB>allowed-genre-set` line per artist, seeded with the artist's majority genre, with `#` comments flagging artists whose albums disagree. `apply` re-scans and calls `retag.py` on every album whose genre falls outside its artist's allowed set, collapsing it to the canonical (first) genre. The allowed-set (append `; Other Genre`) protects artists who legitimately span genres; blank a row to skip an artist. Keys on the normalized artist **tag**, not folder name. `--dry-run`, append-only log (`<directory>/genre_tidy.log`), idempotent; re-running `build` preserves edits and only appends new artists. Aimed at messy libraries; a cleanly tagged one reports everything compliant. Tested by `tests/test_genre_tidy.py`.
