...
Tests live in top-level `tests/` (stdlib `unittest`, no third-party deps); run them from the repo root with `python -m unittest discover` (78 tests). They cover the pure helpers (rating/key normalization, duration clustering, JPEG/PNG header parsing, filename cleanup, track-number parsing, decode classification) and the mode functions end to end against a committed fixture library (`tests/fixtures/library/`, regenerated by `tests/fixtures/build.py` with ffmpeg + mutagen). Integrity modes that shell out to flac/ffmpeg are not exercised; their `classify_decode` brain is unit-tested instead. When changing a covered helper or mode, update its test in the same pass.
